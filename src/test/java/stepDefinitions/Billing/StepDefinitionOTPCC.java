...
public class StepDefinitionOTPCC extends RestUtils {
	PostLoginPayment postLoginPayment =new PostLoginPayment();
	TestRequestBuilder data = new TestRequestBuilder();
	String transID = null;
	JsonUtil jsUtil_OTP_CC = new JsonUtil(Filepaths.testDataScpJsonFilepath,
			"oneTimePaymentCC.json");
	...
	public void add_OneTimePayment_Payload_with_and_and_and_and_and(String cardType,
																	String ccAccountNum,
																	String ccExp,
			String cardCVV, String amount, String serviceAccountNumber) {
		res = given().spec(requestSpecification()).body(data
				.oneTimePaymentCCPayload(ccAccountNum, ccExp, cardCVV, amount,
	...
	public void the_API_call_got_success_with_status_code(String arg1) {
		System.out.println("hello " + response.getStatusCode());
	...
	public void user_validate_the_from_the_Response_Message(String keyValue, String expValue) {
		Assert.assertEquals(expValue, getJsonPath(response, keyValue));
	}
	...
	public void user_validate_for_with_DB_details(String keyValue, String statusCode,
		...
		if (statusCode.equals("200")) {
			...
			Assert.assertEquals(amt, OneTimePaymentAPI.getPaymentFrom_ChaseDB(transID)
					.get("Amount"));
	...
	public void user_validate_the_in_DataBase_paymentRecons_Table(String cardType) {
		switch (cardType) {
		case "Credit Card - VISA":
			Assert.assertEquals(OneTimePaymentAPI.getPaymentFrom_ChaseDB(transID)
					.get("PaymentMethodSubType"), "4");
			break;
		case "Credit Card - MASTER CARD":
			Assert.assertEquals(OneTimePaymentAPI.getPaymentFrom_ChaseDB(transID)
					.get("PaymentMethodSubType"), "3");
			break;
		case "Credit Card - Discover":
			Assert.assertEquals(OneTimePaymentAPI.getPaymentFrom_ChaseDB(transID)
					.get("PaymentMethodSubType"), "2");
			break;
		}
	}
	...
	public void user_add_Credit_Card_for_CardType(DataTable table) throws Throwable {
		...
		ccp.user_add_credit_card_with_details_to_profile_and_and_and(data.get(4).get(1),
				data.get(3).get(1),
	...
	public void user_Make_payment_with_card_type_with_amount(String cardType, String amt) throws Throwable {
		String custRefNo = OneTimePaymentAPI.getPaymentProfile_ChaseDB(data.getPaymentProfile()
						.getCardName(), String.valueOf(data.getPaymentProfile().getUserID()),
				data.getPaymentProfile().getUtilityAccountNumber()).get("customerRefNum");
		String bodyForTokenizePayment = postLoginPayment.makeTokenizePayment(data.getPaymentProfile()
						.getUtilityAccountNumber(), custRefNo,amt, "Card",
				String.valueOf(data.getPaymentProfile().getUserID()));
