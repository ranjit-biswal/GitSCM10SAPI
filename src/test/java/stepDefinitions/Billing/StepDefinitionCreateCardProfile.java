...
public class StepDefinitionCreateCardProfile extends RestUtils {
	TestRequestBuilder data = new TestRequestBuilder();
	public static ResultSet rs;
	JsonUtil jsUtil = new JsonUtil(Filepaths.testDataScpJsonFilepath, "createProfileCC.json");
	...
	public void deleteCardProfile(String userID) throws Exception {
		ResultSet rs1 = DataBaseUtil.getResultSetPaymentDbChase(SqlQuery.getCustRefNoCount(userID));
		rs1.next();
		String countRefNum = rs1.getString("custRefNumber");
		if (!countRefNum.equals("0")) {
			try {
				rs = DataBaseUtil.getResultSetPaymentDbChase(SqlQuery.getCustRefnumber(userID));
				while (rs.next()) {
